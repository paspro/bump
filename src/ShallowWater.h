...
class ShallowWater
{
  ...
  public:
     ...
     */
    ShallowWater& operator=(const ShallowWater&) = delete;

  public:
    /*!
     * @brief Compute the Bernoulli coefficients i.e. the coefficient of the cubic form
     * of the Bernoulli equation a*h^3 + b*h^2 + c*h + d = 0 for the shallow
     * water flow between an inflow and an outflow location.
     *
     * @param q_in       The inflow water rate [m²/s].
     * @param h_out      The water height at the outflow [m].
     * @param z_in       The elevation at the inflow [m].
     * @param z_out      The elevation at the outflow [m].
     * @param g          Gravity [m/s²].
     *
     * @return The coefficients of the cubic form of the Bernoulli equation.
     */
    static std::tuple<double, double, double, double>
    bernoulli_coefficients(double q_in, double h_out, double z_in, double z_out, double g);
     ...
     *
     * @return A vector of complex numbers representing the roots of the cubic equation.
     */
     ...
     * @param h_near         The height of water at the previously computed location [m].
     *
     * @return The height of water [m].
     */
    static double find_solution(const std::vector<std::complex<double>>& roots, double h_near);
