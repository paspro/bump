...
void Bump::initialise_geometry(double length, std::size_t n_cells)
{
    //
...
void Bump::compute_subcritical_case(
    double q_in, double h_out, double g, const std::string& filename, bool screen_output)
{
    //
    ...
    for (auto n = Bump::n_cells - 1; n >= 0; n--)
    {
        //
...
void Bump::compute_transcritical_no_shock_case(double q_in,
                                               double g,
                                               ...
                                               bool screen_output)
{
    ...
    for (auto n = Bump::n_max - 1; n >= 0; n--)
    {
        ...
        //
        const auto coeffs =
            ShallowWater::bernoulli_coefficients(q_in, h_middle, Bump::z[n], Bump::z_max, g);
        //
        // The bump height solutions.
        //
    ...
    for (auto n = Bump::n_max + 2; n <= Bump::n_cells; n++)
    {
        //
        // The coefficients of the Bernoulli cubic equation to solve
        // h^3 + a*h^2 + b*h + c = 0.
    ...
    {
        std::cout << std::fixed << std::setprecision(6);
        std::cout << "Maximum bump height                 = " << Bump::z_max << std::endl;
    ...
    for (int n = 1; n <= n_cells; n++)
    {
        ...
        if (!filename.empty())
        {
            ...
            transcritical_no_shock_file << Bump::x[n] << "   " << h_water[n] << std::endl;
        }
    }
    //
    // Close the file.
...
void Bump::compute_transcritical_shock_case(
    double q_in, double h_out, double g, const std::string& filename, bool screen_output)
{
    //
    // Transcritical No-Shock flow over a bump.
    ...
    //
    for (auto n = Bump::n_cells - 1; n > n_shock; n--)
    {
        //
        // The coefficients of the Bernoulli cubic equation to solve
        // a*h^3 + b*h^2 + c*h + d = 0.
    ...
    if (screen_output)
    {
        ...
        std::cout << "Shock height minus                  = " << h_minus << std::endl;
        std::cout << "Shock height plus                   = " << h_plus << std::endl;
        std::cout << "==============================================" << std::endl;
        std::cout << "x (m)     Z (m)     height (m)" << std::endl;
    }
    for (int n = 0; n <= Bump::n_cells; n++)
    ...
    for (int n = 1; n <= Bump::n_cells; n++)
    {
        ...
        if (!filename.empty())
        {
            ...
            transcritical_shock_file << Bump::x[n] << "   " << h_water[n] << std::endl;
        }
    }
    //
    // Close the file.
